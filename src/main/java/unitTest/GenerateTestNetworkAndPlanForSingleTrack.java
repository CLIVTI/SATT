...
//         n_1       n_2              n_3          n_4
	...
	public void createNetwork(String outputNetworkFile, String outputPlanFile){
		...
		links.add(fac.createLink(Id.createLinkId("l_2_AB"), node2, node3));
		links.add(fac.createLink(Id.createLinkId("l_2_BA"), node3, node2));
		...
		Activity work = population.getFactory().createActivityFromLinkId("home", links.get(4).getId());
		...
		// third add a person-trip from node1 to node4 but 10 sec departure time shift
				...
				// first add a person-trip from node1 to node8
				Activity home3 = population.getFactory().createActivityFromLinkId("home", links.get(5).getId());
